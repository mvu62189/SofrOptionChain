...
def load_and_process_data_for_all_timestamps(model_engine):
    ...
    # --- Initialize a log for skipped files ---
    ...
    for f in all_files:
        ...
        if not df.empty:
            df['__source_file__'] = f  # Add the source file path for later use
            df_list.append(df)
    ...
    for ts, df_snapshot in full_df.groupby('snapshot_ts'):
        ...
        for expiry, df_expiry in df_snapshot.groupby('expiry_date'):
            ...
            # --- Modify file source name for better logging ---
            ...
            if not res or not res.get('params_fast'):
                # --- Log skipped files instead of printing warnings ---
            ...
            # --- MAKE SABR IV CALCULATION MODEL-AWARE ---                      ### REVISIT ### ## LOGIC ## ## SCALE ##
...
with st.sidebar:
    ...
    # --- MODEL ENGINE TOGGLE ---
    model_engine_display = st.radio("Select Pricing Model",
...
if not all_data:
    ...
    # --- Still show the skipped files tab even if all data fails ---
    if skipped_files:    
